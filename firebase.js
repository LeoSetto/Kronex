...
export default function AuthProvider({ children }) {
  ...
  const doRegister = () => {
    ...
    createUserWithEmailAndPassword(auth, email, password)
      ...
      .catch(e => setError(errMsg[e.code] || e.message)).finally(() => setBusy(false));
  };
  ...
  const doGoogle = () => {
    ...
    fn(auth, googleProvider).catch(e => { if (e.code !== "auth/popup-closed-by-user") setError(errMsg[e.code] || e.message); }).finally(() => setBusy(false));
  };

  const doLogout = () => signOut(auth);

  if (loading) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#080C12"}}><KronexSymbol size={50}/></div>;
  ...
  const I = (props) => <input {...props} style={{width:"100%",padding:"14px 16px",background:"#0A1018",border:"1px solid #1A2A3A",borderRadius:10,color:"#E0F0F8",fontSize:16,fontFamily:"'DM Sans',sans-serif",outline:"none",marginBottom:12,boxSizing:"border-box",WebkitAppearance:"none",...(props.xstyle||{})}} />;
  const B = ({primary,children:c,...p}) => <button {...p} type="button" style={{width:"100%",padding:"14px 18px",borderRadius:10,fontSize:15,fontWeight:primary?700:500,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",border:primary?"none":"1px solid #1A2A3A",marginBottom:10,transition:"all .2s",WebkitAppearance:"none",background:primary?"linear-gradient(135deg,#22D3EE,#0891B2)":"#0A1018",color:primary?"#000":"#E0F0F8",boxShadow:primary?"0 2px 16px rgba(34,211,238,.3)":"none",opacity:p.disabled?.6:1,...(p.xstyle||{})}}>{c}</button>;
  ...
  return (
    <>
      ...
      <div style={{background:"#0E1420",border:"1px solid #1A2A3A",borderRadius:16,padding:"32px 24px",width:"100%",maxWidth:420,textAlign:"center"}}>
        ...
        {step === "login" && <>
          <I type="email" inputMode="email" autoComplete="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} enterKeyHint="next"/>
          <I type="password" autoComplete="current-password" placeholder="Senha" value={password} onChange={e=>setPassword(e.target.value)} enterKeyHint="go" onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();doLogin();}}}/>
          <B primary onClick={doLogin} disabled={busy}>{busy?"Aguarde...":"Entrar"}</B>
          ...
          <B onClick={doGoogle} disabled={busy}><span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><svg width="18" height="18" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>Entrar com Google</span></B>
        </>}

        {step === "register" && <>
          <I type="text" autoComplete="name" placeholder="Seu nome" value={name} onChange={e=>setName(e.target.value)} enterKeyHint="next"/>
          <I type="email" inputMode="email" autoComplete="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} enterKeyHint="next"/>
          <I type="password" autoComplete="new-password" placeholder="Senha (mínimo 6)" value={password} onChange={e=>setPassword(e.target.value)} enterKeyHint="go" onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();doRegister();}}}/>
          <B primary onClick={doRegister} disabled={busy}>{busy?"Aguarde...":"Criar Conta"}</B>
